(function() {
	...
	let gameInit = function() {
		...
		let description =
			'<p>Click or press Enter to reveal a monster behind the chosen door.</p>' +
			...
			monsters
				.map(function(monster, index) {
					return (
						...
						index +
						'"><img title="Click door to reveal the monster" alt="Picture of a door"' +
						'src="img/door.svg"' +
						'/></button>' +
						'</div>'
					);
	...
	let handleClicks = function(e) {
		...
		let buttonTag = e.target.closest('[data-image-id]');
		if (buttonTag) {
			let id = buttonTag.getAttribute('data-image-id');
			buttonTag.parentNode.innerHTML =
				'<img alt="Picture of a monster"' + 'src="img/' + monsters[id] + '.svg"' + '/>';

			trackGame(monsters[id]);
	...
	let trackGame = function(img) {
		...
		if (img === 'sock') {
			...
			// Increment number of monsters found
			monsterCount++;
			// If all monsters are found, (11 total) then render win UI
